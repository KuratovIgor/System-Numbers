...
namespace LibSystemNumbers
{
    public static class Number
    {
        ...
        private static Dictionary<char, int> dictionaryOfNumbers = new Dictionary<char, int>(26) {
                {'A', 10}, {'B', 11}, {'C', 12}, {'D', 13}, {'E', 14}, {'F', 15}, {'G', 16}, {'H', 17}, {'I', 18},
                {'J', 19}, {'K', 20}, {'L', 21}, {'M', 22}, {'N', 23}, {'O', 24}, {'P', 25}, {'Q', 26}, {'R', 27},
                {'S', 28}, {'T', 29}, {'U', 30}, {'V', 31}, {'W', 32}, {'X', 33}, {'Y', 34}, {'Z', 35}
            };
        //Словарь чисел
        private static Dictionary<int, char> dictionaryOfSymbols = new Dictionary<int, char>(26) {
        ...
        private static char IntToChar(int numbers) 
        {
            ...
            if (numbers <= 9) 
                return (char)(numbers + '0');

            return (char)(numbers + 55);
        }
        ...
        private static bool IsNumberCorrect()
        {
            ...
            if (_number[0] == '.' || _number[length - 1] == '.')
                return false;

            for (int i = 0; i < length; i++) 
            {
                if (CharToInt(_number[i]) >= _basis)
                    return false;                           
       ...
       //Система счисления не может быть меньше 2 и больше 36
        ...
        private static char TranslateToMoreSystems(char number)
        {
            if (number >= 'A' && number <= 'Z')
                return IntToChar(dictionaryOfNumbers[number]);
            
            return dictionaryOfSymbols[CharToInt(number)];                
        }
        ...
        public static char[] TranslateToSystemNumbers(string stringNumbers, int firstBasis, int secondBasis)  
        {
            ...
            { //Проверка на корректность записи введённого числа
